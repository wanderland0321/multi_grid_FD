...
// jacobsolver( Nxy, vdxy[i], vxymatrix[i], vxynewmatrix[i], fvmatrix[i], residualmatrix[i], tolfactor);
void jacobsolver( int Nxy, double dxy, double *vxy, double *vxynew, double *fv, double *vresidual, double tolfactor){
  ...
  k = 0;
  // while(residual2>tolfactor*residualinit){
  while(k<20){
    k = k + 1;
    residual2 = 0.0;
    ...
    for(i=1;i<(Nxy-1);i++){
      for(j=1;j<(Nxy-1);j++){
         ...
         // residual       =  0.25*( vxy[ixyup]+vxy[ixydown]+vxy[ixyleft]+vxy[ixyright]+dxy*dxy*fv[ixy] ) - vxy[ixy];    // 
         vresidual[ixy] =  -residual;      //      there maybe a SIGN correction HERE;
      }
    }
